var app = (function(nx){
    ...
    return{
		...
		getTopology:function(source,destination){
            ...
            if(routingParams.length!=2){
                alert('Source and Destination are missing.');
                return false;
            }
			else if(routingParams[0]==routingParams[1])
			{
				alert('Source and destintion cannot be the same.');
                return false;
			}
			 ...
			 $.ajax({
				...
				success:function(data){
                   ...
                   //shell.start(sampleTopologyData);
                    try {
                        var topologyData={ nodes: mapping.nodes,
                links: mapping.links};
